...
MAX_FPS = 1020
DEFAULT_FPS = 30
STARTING_X = WIDTH/2  # Starting position for all agents
...
class GameAgent:
    def __init__(self, nn_agent):
        ...
        self.x = STARTING_X  # Use fixed starting position
        self.alive = True
        self.score = 0
...
def get_inputs(agent_x, walls, ball_y):
    ...
    if closest_wall:
        inputs[1] = closest_wall["x"] / WIDTH  # Hole left x
        inputs[2] = (closest_wall["x"] + HOLE_WIDTH) / WIDTH  # Hole right x
        inputs[3] = (ball_y - closest_wall["y"]) / HEIGHT  # Distance to wall
...
while running:
    ...
    while running:
        ...
        # Spawn Walls with random holes
        if game_state["frame_count"] % SPAWN_INTERVAL == 0:
            hole_x = random.randint(50, WIDTH - HOLE_WIDTH - 50)  # Random hole position
        ...
        for game_agent in game_agents:
            if game_agent.alive:
                ...
                # Collision Detection
                for wall in game_state["walls"]:
                    if wall["y"] <= HEIGHT - 50 <= wall["y"] + WALL_HEIGHT:  # Check if ball's Y intersects with wall
                        if not (wall["x"] < game_agent.x < wall["x"] + HOLE_WIDTH - BALL_SIZE):  # Check if ball is not in hole
